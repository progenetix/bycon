...
export function getLengthOnRef(cigar) {
...
export function getLength(cigar) {
    const cigarOps = parseCigar(cigar);
    let length = 0;
    for (let i = 0; i < cigarOps.length; i += 2) {
        const len = +cigarOps[i];
...
export function getLengthSansClipping(cigar) {
    const cigarOps = parseCigar(cigar);
    let length = 0;
    for (let i = 0; i < cigarOps.length; i += 2) {
        const len = +cigarOps[i];
...
export function featurizeSA(SA, id, strand, readName, normalize) {
    return ((SA === null || SA === void 0 ? void 0 : SA.split(';').filter(aln => !!aln).map((aln, index) => {
        ...
        const saClipPos = getClip(saCigar, (normalize ? strand : 1) * saStrandNormalized);
