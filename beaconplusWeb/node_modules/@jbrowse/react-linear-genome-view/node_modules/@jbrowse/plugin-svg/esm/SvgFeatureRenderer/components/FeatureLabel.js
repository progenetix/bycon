...
const FeatureLabel = observer(function ({ text, x, y, region, reversed, bpPerPx, exportSVG, feature, viewParams, color = 'black', fontHeight = 11, featureWidth = 0, allowedWidthExpansion = 0, displayModel = {}, }) {
    const totalWidth = featureWidth + allowedWidthExpansion;
    const measuredTextWidth = measureText(text, fontHeight);
    const params = isStateTreeNode(displayModel) && isAlive(displayModel) && !exportSVG
    ...
    return labelVisible ? (React.createElement("text", { x: x, y: y + fontHeight, fill: color === '#f0f' ? stripAlpha(theme.palette.text.primary) : color, fontSize: fontHeight }, measuredTextWidth > totalWidth
