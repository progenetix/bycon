...
test('throws if stanzas have mismatched keys', () => {
    expect(() => new RaFile(fs.readFileSync('test/mismatchKeys.ra', 'utf8'))).toThrow(/must have the same key/);
});
test('throws if stanzas have duplicate names', () => {
    expect(() => new RaFile(fs.readFileSync('test/duplicateName.ra', 'utf8'))).toThrow(/duplicate stanza name/);
});
