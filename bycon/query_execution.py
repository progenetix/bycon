...
def execute_bycon_queries(**kwargs):
    ...
    if q_coll_name in exe_queries:
        ...
        query_results["variants::_id"] = mongo_coll.distinct( "_id", exe_queries[ q_coll_name ] )
        query_results[ "callsets::id" ] = mongo_coll.distinct( "callset_id", exe_queries[ q_coll_name ] )
    ...
    """podmd
    All above query pipelines provide `callsets::_id` values which are then
    used to retrieve the matching biosample `id` and `_id` values.
    podmd"""
