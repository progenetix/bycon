...
class BeaconResponseMeta:
    ...
    def populatedMeta(self):
        if self.data_response:
            self.response_meta = always_merger.merge(
                self.response_meta, self.data_response.get("meta", {})
            )
    ...
    def __meta_add_parameters(self):
        ...
        info = self.entity_defaults["info"].get(
            "content", {"api_version": "___none___"}
        )
    ...
    def __meta_add_received_request_summary_parameters(self):
        if self.record_queries:
            self.response_meta.update(
                {
                    "info": always_merger.merge(
                        self.response_meta.get("info", {}),
                        {"original_queries": self.record_queries},
                    )
                }
            )
        ...
        for q in ["cohort_ids", "cyto_bands", "chro_bases"]:
            if q in BYC_PARS:
                r_r_s.update(
                    {
                        "request_parameters": always_merger.merge(
                            r_r_s.get("request_parameters", {}), {q: BYC_PARS.get(q)}
                        )
                    }
                )
...
class BeaconErrorResponse:
    ...
    def __init__(self):
        ...
        self.informational_error_response = ByconSchemas(
            "beaconInformationalResponseMeta", ""
        ).get_schema_instance()
...
class BeaconInfoResponse:
    ...
    def __response_add_returned_schemas(self):
        if "info" not in self.response_entity_id:
            return
...
class BeaconDataResponse:
    ...
    def resultsetResponse(self):
        ...
        prdbug(dbm)
        if "beaconResultsetsResponse" not in self.response_schema:
            return
    ...
    def collectionsResponse(self):
        if "beaconCollectionsResponse" not in self.response_schema:
            return
        ...
        self.data_response.get("meta", {}).get("received_request_summary", {}).pop(
            "include_resultset_responses", None
        )
        return self.data_response
    ...
    def filteringTermsResponse(self):
        if "beaconFilteringTermsResponse" not in self.response_schema:
            return
    ...
    def aggregationTermsResponse(self):
        if "beaconAggregationConceptsResponse" not in self.response_schema:
            return
    ...
    def __resultSetResponse_force_autorized_granularities(self):
        ...
        for rs in self.data_response["response"]["result_sets"]:
            ...
            if "record" not in rs_granularity:
    ...
    def __collections_response_remap_cohorts(self):
        if "cohort" not in BYC.get("response_entity_id", "___none___"):
            return
        pop_keys = [
            "_id",
            "frequencymap",
    ...
    def __collections_response_update_summaries(self):
        if "beaconCollectionsResponse" not in self.response_schema:
            return
        if "response" not in self.data_response:
            return
...
class ByconFilteringTerms:
    ...
    def __filtering_terms_query(self):
        ...
        elif "withpubmed" not in self.special_mode:
    ...
    def __return_filter_resources(self):
        ...
        res_schema = ByconSchemas(
            "beaconFilteringTermsResults", "$defs/Resource"
        ).get_schema_instance()
        for c_t in collation_types:
            if c_t not in f_d_s:
                continue
            f_d = f_d_s[c_t]
            if "ontologyTerm" not in f_d.get("type"):
                continue
            r = {}
...
class ByconCollections:
    ...
    def __collections_return_datasets(self):
        if "dataset" not in self.response_entity_id:
            return
    ...
    def __collections_return_cohorts(self):
        if "cohort" not in self.response_entity_id:
            return
...
class ByconResultSets:
    def __init__(self):
        self.datasets_results = (
            dict()
        )  # the object with matched ids per dataset, per h_o
        self.datasets_data = (
            dict()
        )  # the object with data of requested entity per dataset
        self.datasets_aggregations = (
            dict()
        )  # the object with aggregations of requested entity per dataset
    ...
    def __create_empty_result_sets(self):
        r_set = ByconSchemas(
            "beaconResultsets", "$defs/ResultsetInstance"
        ).get_schema_instance()
        r_sets = []
        for ds_id in BYC["BYC_DATASET_IDS"]:
            ds_rset = r_set.copy()
    ...
    def __populate_result_sets(self):
        ...
        for i, r_set in enumerate(self.result_sets):
            ...
            if self.returned_granularity == "aggregated":
                self.result_sets[i].update(
                    {"results_aggregation": self.datasets_aggregations.get(ds_id, [])}
                )

        ds_v_duration = datetime.now() - ds_v_start
        dbm = (
            f"... __populate_result_sets needed {ds_v_duration.total_seconds()} seconds"
        )
        prdbug(dbm)
...
class ByconHO:
    ...
    def __process_handover(self, h_o_t):
        ...
        h_o_r = {
            ...
            "info": {"content_id": h_o_t, "count": target_count},
    ...
    def __handover_create_url(self, h_o_defs, accessid):
        if not (addr := h_o_defs.get("script_path_web")):
            return ""
        url = f"{addr}?datasetIds={self.dataset_id}&accessid={accessid}".replace(
            "___BEACON_ROOT___", BEACON_ROOT
        )
